...
mydb=sqlite3.connect("D:/ans management software/database/db/mydb.db")
mycur=mydb.cursor()
root=Tk()
...
def add_to_invent1():
            ...
            #mycur.execute(f"create table {self.cat_var.get()}(date_of_entry date,amt_added int,amt_decresed int,reason varchar(20),total int")
                try:
                    mycur.execute(f"insert into inven values({dateentry1},'{nameenntry1}','{catpatno1}',{numentry1})")
                    mydb.commit()
                ...
                #display.insert(END, "date         name     cno  amt")
                    ...
                    for x in range(c):
                        t = str(result9[x][0])
                        l = str(t[0]) + str(t[1]) + str(t[2]) + str(t[3]) + str("-") + str(t[4]) + str(t[5]) + str("-") + str(t[6]) + str(t[7])
                        f.write("\t\t" + str(l))
                    ...
                    if error==f"UNIQUE constraint failed: inven.cat_pat_no":
                        tkinter.messagebox.showerror("error","This GRN number already exists")
